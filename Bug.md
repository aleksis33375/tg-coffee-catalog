...
Статус: ✅ исправлен (2026-04-19) — добавлены date-picker «от/до» + кнопка «Показать»; Compare (две линии) отложен
...
## Аудит 2 (2026-04-19) — новые баги

### Исправлены в том же цикле

**Б-А84 — `escapeHtml()` в лейблах Chart.js ломает спецсимволы**  
Файл: [admin/admin.js](admin/admin.js)  
Простыми словами: Chart.js рисует лейблы диаграмм на `<canvas>` — это не HTML. Если передать туда строку с `escapeHtml()`, в легенде диаграммы отобразится `&amp;` вместо `&`. Пользователь увидит мусорные символы в названиях категорий.  
Что сделать: убрать `escapeHtml()` из массивов лейблов Chart.js — canvas не уязвим к XSS.  
Статус: ✅ исправлен (2026-04-19)

---

**Б-А85 — `parsePeriodRange` не валидирует from/to → 500 на невалидной дате**  
Файл: [backend/routes/admin.js](backend/routes/admin.js)  
Простыми словами: если кто-то передаёт в запрос `?from=abc&to=xyz`, функция вызывает `new Date('abcT00:00:00+03:00').toISOString()` — это бросает исключение. Сервер отвечает 500 без понятного сообщения.  
Что сделать: проверить `isNaN(date)` перед `.toISOString()`, при невалидных датах откатиться к дефолтному периоду.  
Статус: ✅ исправлен (2026-04-19)

---

**Б-А87 — Статус `cancelled` отсутствует в `STATUS_LABELS` → в бейдже заказа отображается `undefined`**  
Файл: [admin/admin.js](admin/admin.js)  
Простыми словами: бэкенд принимает и хранит статус `cancelled` (отменён), но в словаре STATUS_LABELS его нет. При показе отменённого заказа в бейдже отобразится слово `undefined` — некрасиво и непонятно пользователю.  
Что сделать: добавить `cancelled: 'Отменён'` в объект `STATUS_LABELS`.  
Статус: ✅ исправлен (2026-04-19)

---

**Б-А88 — Поле `source` в POST /customers не ограничено по длине**  
Файл: [backend/routes/public.js](backend/routes/public.js)  
Простыми словами: при регистрации клиента поле `source` берётся из тела запроса напрямую без ограничения длины. Злоумышленник может отправить запрос с `source = "A".repeat(100000)` — это запишет в БД мегабайтную строку. Соседние поля `first_name` и `username` уже ограничены, `source` пропущено.  
Что сделать: добавить `.slice(0, 50)` к `source`, как уже сделано для остальных строковых полей.  
Статус: ✅ исправлен (2026-04-19)

---

**Б-А86 — `/analytics/by-category` молча игнорирует ошибку запроса к menu_items**  
Файл: [backend/routes/admin.js](backend/routes/admin.js)  
Простыми словами: если таблица `menu_items` временно недоступна, ошибка glотается и диаграмма по категориям показывает всё как «Без категории». Администратор не видит никакой ошибки и не понимает, почему данные неверные.  
Что сделать: логировать `menuRes.error` в консоль (не критично возвращать 500 — данные частично доступны).  
Статус: ✅ исправлен (2026-04-19)

---

**Б-А89 — Список последних заказов навсегда «Загрузка...»**  
Файлы: [backend/routes/admin.js](backend/routes/admin.js), [admin/admin.js](admin/admin.js)  
Простыми словами: запрос `/admin/orders` использовал специальный синтаксис Supabase для подтягивания имён клиентов — `customers(first_name, username)`. Это требует, чтобы в базе данных был прописан внешний ключ (FK). Если FK не прописан, Supabase возвращает ошибку. Ошибка тихо уходила в консоль, а список заказов так и висел с «Загрузка...».  
Что сделать: убрать зависимость от FK — имена клиентов подтягивать отдельным запросом и склеивать на сервере. В случае любой ошибки показывать внятное сообщение вместо вечного спиннера.  
Статус: ✅ исправлен (2026-04-19)

---

**Б-А90 — История рассылок вечно «Загрузка...» при ошибке API**  
Файл: [admin/admin.js](admin/admin.js)  
Простыми словами: раздел «История рассылок» инициализируется с текстом «Загрузка...». Если запрос на сервер завершается ошибкой, ошибка уходит только в консоль — экран не обновляется. Итог: администратор видит вечный спиннер и не понимает, что пошло не так.  
Что сделать: в блоке catch обновлять текст блока с понятным сообщением об ошибке.  
Статус: ✅ исправлен (2026-04-19)

---

**Б-А91 — Топ позиций вечно «Загрузка...» при ошибке API**  
Файл: [admin/admin.js](admin/admin.js)  
Простыми словами: та же проблема что в Б-А89/90, но в блоке «Топ позиций». При ошибке загрузки блок остаётся с надписью «Загрузка...» и не объясняет причину.  
Что сделать: в блоке catch обновлять текст блока с сообщением об ошибке.  
Статус: ✅ исправлен (2026-04-19)

---

## Аудит 3 (2026-04-20) — новые баги

### Средние

**Б-А92 — Топ позиций и пиковые часы считают отменённые заказы**
Файл: [backend/routes/admin.js](backend/routes/admin.js)
Простыми словами: в блоке «Топ позиций» учитываются все заказы — в том числе отменённые. Если клиент заказал 10 латте и отменил — они всё равно попадают в рейтинг. Топ раздувается фиктивными данными, нельзя доверять этим цифрам.
Что сделать: добавить фильтр `.eq('status', 'done')` к запросу заказов в `/analytics/top-items`.
Статус: ✅ исправлен (2026-04-20)

---

**Б-А93 — CSV-экспорт клиентов молча обрезается на 1000 записях**
Файл: [backend/routes/admin.js](backend/routes/admin.js)
Простыми словами: кнопка «Скачать CSV» отдаёт клиентскую базу, но запрос к Supabase без явного лимита ограничен 1000 строками по умолчанию. Если клиентов больше 1000 — остальные просто не попадут в файл, без какого-либо предупреждения. Администратор думает, что получил полную базу, а на самом деле только часть.
Что сделать: реализовать постраничную выборку через `.range()` в цикле (аналогично `fetchAllPaginated` в cron.js) или установить явный высокий лимит.
Рекомендация: **средний приоритет** — требует решения по архитектуре (потоковая отдача vs буфер).
Статус: ⬜ не исправлен
...
**Б-А94 — `loadDrilldown` вставляет дату в innerHTML без escapeHtml**
Файл: [admin/admin.js](admin/admin.js)
Простыми словами: при клике на точку графика в заголовке всплывающей панели появляется дата «Заказы за 2026-04-19». Дата приходит с нашего сервера в формате `YYYY-MM-DD` и по факту безопасна, но технически вставляется без экранирования. Если значение когда-либо придёт с неожиданными символами — возможен XSS.
Что сделать: обернуть `date` в `escapeHtml()` и `encodeURIComponent()` при подстановке в URL.
Статус: ✅ исправлен (2026-04-20)
...
| Б-А75 | 🟡 Средний | Дашборд: нет среднего чека и счётчика новых клиентов в статистике | ✅ исправлен |
| Б-А76 | 🟡 Средний | Дашборд: нет произвольного диапазона дат и кнопки «Сравнить» | ✅ исправлен |
| Б-А77 | 🟡 Средний | Дашборд: фильтр периода применяется только к графику, а не ко всем блокам | ✅ исправлен |
| Б-А78 | 🔵 Низкий | Дашборд: график выручки столбчатый, по схеме должен быть линейный | ✅ исправлен |
| Б-А79 | 🟡 Средний | Дашборд: нет диаграммы источников трафика (откуда приходят клиенты) | ✅ исправлен |
| Б-А80 | 🟡 Средний | Дашборд: нет графика пиковых часов для администратора | ✅ исправлен |
| Б-А81 | 🟡 Средний | Дашборд: нет диаграммы выручки по категориям (пончик) | ✅ исправлен |
| Б-А82 | 🟡 Средний | Дашборд: нет воронки конверсии visitor → buyer | ✅ исправлен |
| Б-А83 | 🟡 Средний | Дашборд: нет drill-down панели при клике на график | ✅ исправлен |
| Б-А84 | 🟡 Средний | escapeHtml() в Chart.js labels ломает спецсимволы (canvas не HTML) | ✅ исправлен |
| Б-А85 | 🟡 Средний | parsePeriodRange не валидирует from/to → 500 на невалидной дате | ✅ исправлен |
| Б-А86 | 🔵 Низкий | /analytics/by-category молча игнорирует ошибку menu_items | ✅ исправлен |
| Б-А87 | 🔵 Низкий | STATUS_LABELS не содержит 'cancelled' → undefined в бейдже заказа | ✅ исправлен |
| Б-А88 | 🟡 Средний | source в POST /customers без ограничения длины → мусор в БД | ✅ исправлен |
| Б-А89 | 🔴 Критический | Список заказов вечно «Загрузка...» из-за Supabase FK join + молчащий catch | ✅ исправлен |
| Б-А90 | 🟡 Средний | История рассылок вечно «Загрузка...» при ошибке API | ✅ исправлен |
| Б-А91 | 🟡 Средний | Топ позиций вечно «Загрузка...» при ошибке API | ✅ исправлен |
| Б-А92 | 🟡 Средний | Топ позиций и пиковые часы считают отменённые заказы | ✅ исправлен |
| Б-А93 | 🟡 Средний | CSV-экспорт клиентов молча обрезается на 1000 записях | ⬜ не исправлен |
| Б-А94 | 🔵 Низкий | loadDrilldown вставляет дату в innerHTML без escapeHtml | ✅ исправлен |
