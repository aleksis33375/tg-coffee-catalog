...
```sql
id              INTEGER PRIMARY KEY
barista_action  TEXT NOT NULL   -- 'cup_marked' | 'discount_applied' | 'status_changed' | 'birthday_set' | 'menu_edited'
...
| Метод | URL | Что делает |
|---|---|---|
| POST | `/api/barista/login` | Проверяет PIN, возвращает JWT с ролью barista |
...
| PUT | `/api/admin/barista/pin` | Сменить PIN бариста |
| GET | `/api/admin/barista/log` | Лог всех действий бариста: что отметил, когда, к какому заказу и клиенту |
